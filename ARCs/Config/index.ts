...
import { Provider, jsonData, wallet } from "../type";
...
const Config = (p: { testnet?: boolean } = { testnet: false }) => {
  const { testnet } = p;
  setStdlibOpts(!testnet);

  const reach = stdlib();
  const Provider = async (): Promise<Provider> => await reach.getProvider();

  const ARC_69_Config = async (
    wallet: wallet,
    Provider: Provider,
    prop: {
      assetId: number;
    ...
    const _note: z.infer<typeof ZodObj> = _data;
  ...
  const Bulk_ARC_69_Config = async (
    wallet: wallet,
    Provider: Provider,
    options: {
      type: "json" | "csv";
    ...
    let failures: jsonData & { reason: unknown }[] = [];
    const { type, jsonData = [] as jsonData } = options;
    let data: jsonData;
